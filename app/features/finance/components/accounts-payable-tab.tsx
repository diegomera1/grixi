...
function KPIWithTooltip({
  label, value, icon: Icon, color, bgClass,
}: {
  label: string; value: string; icon: typeof DollarSign; color: string; bgClass: string;
}) {
  const [showTooltip, setShowTooltip] = useState(false);
  const tooltipText = KPI_TOOLTIPS[label];

  return (
    <div className="rounded-xl border border-border bg-surface p-4 relative">
            ...
            <AnimatePresence>
              {showTooltip && (
                <motion.div
                  ...
                  className="absolute right-0 top-full z-50 mt-1 w-64 rounded-xl border border-border bg-elevated p-3 shadow-xl"
                >
                  <p className="text-[11px] leading-relaxed text-muted-foreground">{tooltipText}</p>
                </motion.div>
              )}
            </AnimatePresence>
          </div>
...
export function AccountsPayableTab({
  ...
  const byMethod = useMemo(() => {
    ...
    const methodLabels: Record<string, string> = {
      transfer: "Transferencia", check: "Cheque", cash: "Efectivo",
      credit_card: "T. Crédito", direct_debit: "Débito",
    };
  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPIWithTooltip label="Total Facturas" value={formatCurrencyCompact(convert(totalInvoiced), currency)} icon={DollarSign} color="text-rose-500" bgClass="bg-rose-500/10" />
        <KPIWithTooltip label="Pagado" value={formatCurrencyCompact(convert(totalPaid), currency)} icon={TrendingDown} color="text-emerald-500" bgClass="bg-emerald-500/10" />
        <KPIWithTooltip label="Pendiente" value={formatCurrencyCompact(convert(outstanding), currency)} icon={Clock} color="text-amber-500" bgClass="bg-amber-500/10" />
        <KPIWithTooltip label="DPO" value={`${dpo}d`} icon={CalendarCheck} color="text-violet-500" bgClass="bg-violet-500/10" />
      </div>
      ...
      <div className="grid grid-cols-2 gap-3">
        <div className="rounded-xl border border-emerald-500/20 bg-emerald-500/5 p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold font-mono text-emerald-400">{formatCurrencyCompact(convert(totalIncome), currency)}</p>
          </div>
        </div>
        <div className="rounded-xl border border-rose-500/20 bg-rose-500/5 p-4 flex items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xl font-bold font-mono text-rose-400">{formatCurrencyCompact(convert(totalExpense), currency)}</p>
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          <h3 className="font-semibold text-sm text-foreground mb-1">Aging de Cuentas por Pagar</h3>
          <p className="text-xs text-muted-foreground mb-4">Antigüedad de obligaciones pendientes</p>
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          <h3 className="font-semibold text-sm text-foreground mb-1">Ingresos vs Egresos</h3>
          <p className="text-xs text-muted-foreground mb-4">Comparación mensual</p>
            ...
            {incomeExpenseData.length > 0 ? (
              <ResponsiveContainer width="100%" height="100%">
                <BarChart data={incomeExpenseData} barGap={2}>
                  ...
                  <RechartsTooltip
                    contentStyle={{ background: "var(--color-surface)", border: "1px solid var(--color-border)", borderRadius: "12px", fontSize: "11px", color: "var(--color-foreground)" }}
              ...
              <div className="flex items-center justify-center h-full text-sm text-muted-foreground">Esperando datos...</div>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-semibold text-sm text-foreground">Top Proveedores</h3>
          </div>
          <p className="text-xs text-muted-foreground mb-4">Mayor deuda pendiente</p>
          <div className="space-y-3">
            {topVendors.map((v, i) => (
              <div key={v.name} className="flex items-center gap-3">
                <span className="w-5 h-5 rounded-full bg-rose-500/15 text-rose-500 flex items-center justify-center text-[10px] font-bold shrink-0">{i + 1}</span>
                <span className="text-xs text-foreground flex-1 truncate">{v.name}</span>
                <span className="text-xs font-mono font-semibold text-rose-500">{formatCurrency(convert(v.amount), currency)}</span>
              </div>
            ))}
            {topVendors.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">Sin proveedores con saldo pendiente</p>
            )}
          </div>
        </div>
        ...
        {/* By Payment Method */}
        <div className="rounded-xl border border-border bg-surface p-5">
          <h3 className="font-semibold text-sm text-foreground mb-1">Por Método de Pago</h3>
          <p className="text-xs text-muted-foreground mb-4">Distribución de pagos</p>
          <div className="space-y-3">
            {byMethod.map((m, i) => (
              <div key={m.method} className="flex items-center gap-3">
                ...
                <span className="text-[10px] font-mono text-muted-foreground w-16 text-right">{formatCurrencyCompact(convert(m.amount), currency)}</span>
              </div>
            ))}
          </div>
        </div>
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-4 py-3 border-b border-border">
          <h3 className="text-sm font-semibold text-foreground">Calendario de Pagos</h3>
          <p className="text-xs text-muted-foreground">Próximas obligaciones por fecha de vencimiento</p>
        </div>
        <div className="divide-y divide-border max-h-[400px] overflow-y-auto">
          {upcomingPayments.map((inv) => {
            ...
            return (
              <button key={inv.id} onClick={() => onSelectTx(inv)} className="w-full flex items-center gap-3 px-4 py-3 hover:bg-muted/30 transition-colors text-left">
                <div className={cn("w-10 h-10 rounded-lg flex flex-col items-center justify-center shrink-0 text-center", isOverdue ? "bg-rose-500/15" : "bg-muted")}>
                  <span className={cn("text-[10px] font-bold uppercase", isOverdue ? "text-rose-500" : "text-muted-foreground")}>
                    {inv.due_date ? new Date(inv.due_date).toLocaleDateString("es-EC", { month: "short" }).slice(0, 3) : "—"}
                  </span>
                  <span className={cn("text-sm font-bold", isOverdue ? "text-rose-500" : "text-foreground")}>
                    {inv.due_date ? new Date(inv.due_date).getDate() : "—"}
                  </span>
                </div>
                <div className="flex-1 min-w-0">
                  <p className="text-xs font-medium text-foreground truncate">{inv.counterparty}</p>
                  <p className="text-[10px] text-muted-foreground">{inv.invoice_number || inv.sap_document_id} · {inv.payment_terms}</p>
                </div>
                <div className="text-right shrink-0">
                  <p className="text-xs font-mono font-semibold text-rose-500">{formatCurrency(convert(inv.amount_usd), currency)}</p>
                  <p className={cn("text-[10px]", isOverdue ? "text-rose-500 font-bold" : "text-muted-foreground")}>
          ...
          {upcomingPayments.length === 0 && (
            <div className="px-4 py-8 text-center text-xs text-muted-foreground">Sin pagos próximos</div>
          )}
        </div>
      </div>
    </div>
