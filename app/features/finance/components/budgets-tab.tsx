    ...
    <div className="space-y-4">
      ...
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Presupuesto Total</p>
          <p className="text-2xl font-bold font-mono text-foreground">{formatCurrencyCompact(convert(totalBudget), currency)}</p>
          ...
          <p className="text-[10px] text-muted-foreground mt-1">{globalPct.toFixed(1)}% ejecutado</p>
        </div>
        <div className="rounded-xl border border-border bg-surface p-4">
          <p className="text-[10px] text-muted-foreground uppercase tracking-wider">Ejecutado</p>
          <p className="text-2xl font-bold font-mono text-rose-500">{formatCurrencyCompact(convert(totalSpend), currency)}</p>
        </div>
        <div className="rounded-xl border border-border bg-surface p-4">
      ...
      <div className="rounded-xl border border-border bg-surface p-5">
        <div className="flex items-center gap-2 mb-1">
          ...
          <h3 className="font-semibold text-sm text-foreground">Centros de Costo → Departamentos</h3>
        </div>
        <p className="text-xs text-muted-foreground mb-4">Presupuesto vs ejecutado por centro — expandir para ver detalle por departamento y responsable</p>
              ...
              <div key={cc.id} className="rounded-lg border border-border overflow-hidden">
                ...
                >
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      <span className="text-[10px] font-mono text-brand bg-brand/10 px-1.5 py-0.5 rounded">{cc.code}</span>
                      <span className="text-xs font-semibold text-foreground">{cc.department}</span>
                      <span className="text-[10px] text-muted-foreground">({departments.length} departamentos)</span>
                    </div>
                  ...
                  <div className="text-right shrink-0">
                    <div className="flex items-center gap-1.5">
                      {isOver ? <AlertTriangle className="w-3 h-3 text-amber-500" /> : <CheckCircle className="w-3 h-3 text-emerald-500" />}
                      <span className="text-[10px] font-mono text-muted-foreground">
                        {formatCurrencyCompact(convert(spent), currency)} / {formatCurrencyCompact(convert(budget), currency)}
                      </span>
                    </div>
                    <span className="text-[10px] font-bold" style={{ color: pct > 80 ? "#EF4444" : pct > 50 ? "#F59E0B" : "#10B981" }}>{pct.toFixed(1)}%</span>
                  </div>
                </button>
                    ...
                    <motion.div initial={{ height: 0, opacity: 0 }} animate={{ height: "auto", opacity: 1 }} exit={{ height: 0, opacity: 0 }} transition={{ duration: 0.2 }} className="overflow-hidden">
                      <div className="px-4 pb-4 pt-1 border-t border-border bg-muted/20">
                        <div className="grid grid-cols-[1fr_auto_auto] gap-x-4 gap-y-0 mb-2">
                          <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold">Departamento</span>
                          <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold">Responsable</span>
                          <span className="text-[9px] text-muted-foreground uppercase tracking-wider font-semibold text-right">Gasto</span>
                        </div>
                        {departments.map((dept, i) => {
                          ...
                          return (
                            <motion.div
                              ...
                              className="grid grid-cols-[1fr_auto_auto] gap-x-4 items-center py-2 border-b border-border/50 last:border-0"
                            >
                              <div className="flex items-center gap-2 min-w-0">
                                <div className="w-2 h-2 rounded-full shrink-0" style={{ backgroundColor: deptColors[i % deptColors.length] }} />
                                <span className="text-xs text-foreground truncate">{dept}</span>
                              </div>
                              <div className="flex items-center gap-1.5">
                                <User size={10} className="text-muted-foreground" />
                                <span className="text-[11px] text-muted-foreground whitespace-nowrap">{responsible}</span>
                              </div>
                              <div className="text-right">
                                <span className="text-xs font-mono font-semibold text-foreground">{formatCurrencyCompact(convert(deptSpend), currency)}</span>
                                <span className="text-[9px] text-muted-foreground ml-1">({deptPct.toFixed(0)}%)</span>
                              </div>
                            </motion.div>
                          );
                        })}
                        <div className="flex items-center justify-between pt-2 mt-1">
                          <span className="text-[10px] font-semibold text-muted-foreground">Total {cc.code}</span>
                          <span className="text-xs font-mono font-bold text-foreground">{formatCurrencyCompact(convert(spent), currency)}</span>
                        </div>
                      </div>
                    </motion.div>
                  )}
