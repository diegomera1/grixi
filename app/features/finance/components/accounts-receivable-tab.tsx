  ...
  return (
    <div className="space-y-4">
      <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
        <KPIWithTooltip label="Total Facturado" value={formatCurrencyCompact(convert(totalInvoiced), currency)} icon={DollarSign} color="emerald" />
        <KPIWithTooltip label="Cobrado" value={formatCurrencyCompact(convert(totalCollected), currency)} icon={TrendingUp} color="cyan" />
        <KPIWithTooltip label="Pendiente" value={formatCurrencyCompact(convert(outstanding), currency)} icon={Clock} color="amber" />
        <KPIWithTooltip label="DSO" value={`${dso}d`} icon={AlertTriangle} color="rose" />
      </div>

      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        {/* Aging */}
        <div className="rounded-xl border border-border bg-surface p-5">
          <h3 className="font-semibold text-sm text-foreground mb-1">Aging de Cartera</h3>
          <p className="text-xs text-muted-foreground mb-4">Antigüedad de cuentas por cobrar</p>
        ...
        <div className="rounded-xl border border-border bg-surface p-5">
          <div className="flex items-center gap-2 mb-1">
            ...
            <h3 className="font-semibold text-sm text-foreground">Top Clientes por Deuda</h3>
          </div>
          <p className="text-xs text-muted-foreground mb-4">Saldos pendientes más altos</p>
          <div className="space-y-3">
            ...
            {topCustomers.length === 0 && (
              <p className="text-xs text-muted-foreground text-center py-4">Sin clientes con saldo pendiente</p>
            )}
          </div>
        </div>
      </div>
      ...
      <div className="rounded-xl border border-border bg-surface overflow-hidden">
        <div className="px-4 py-3 border-b border-border flex items-center justify-between">
          <h3 className="text-sm font-semibold text-foreground">Facturas Pendientes ({pendingInvoices.length})</h3>
        </div>
        <div className="divide-y divide-border max-h-[500px] overflow-y-auto">
          ...
          {pendingInvoices.length === 0 && (
            <div className="px-4 py-8 text-center text-xs text-muted-foreground">Sin facturas pendientes</div>
          )}
        </div>
      </div>
    </div>
