      ...
      <ReactMarkdown
        ...
        components={{
          code({ className, children, ...props }) {
            ...
            if (match) {
              return (
                <div className="my-1.5 overflow-hidden rounded-lg border border-(--border)">
                  <div className="bg-(--bg-muted) px-2 py-1 text-[8px] font-medium uppercase tracking-wider text-(--text-muted)">
                    {match[1]}
                  </div>
            ...
            return (
              <code className="rounded bg-(--bg-muted) px-1 py-0.5 text-[10px] font-mono" {...props}>
                {children}
              </code>
            );
          },
          ...
          thead({ children }) { return <thead className="bg-(--bg-muted) text-(--text-muted)">{children}</thead>; },
          ...
          tr({ children }) { return <tr className="transition-colors hover:bg-(--bg-muted)/50">{children}</tr>; },
