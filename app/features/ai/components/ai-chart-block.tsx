...
function ChartContent({
  type, data, resolvedX, resolvedKeys, chartColors, fullscreen = false,
}: {
...
export function AiChartBlock({ config }: { config: ChartConfig }) {
  ...
  const resolvedKeys = useMemo(() => {
    ...
    const numericKeys = allKeys.filter((k) => k !== xKey && typeof data[0][k] === "number");
    ...
    <>
      <div className="my-3 rounded-xl border border-(--border) bg-(--bg-primary) p-4">
        ...
        <div className="h-64 w-full">
          <ChartContent type={type} data={data} resolvedX={resolvedX} resolvedKeys={resolvedKeys} chartColors={chartColors} />
        </div>
      </div>
        ...
        createPortal(
          <div className="fixed inset-0 z-9999 flex flex-col bg-(--bg-primary)">
            ...
            <div className="flex-1 p-6">
              <ChartContent type={type} data={data} resolvedX={resolvedX} resolvedKeys={resolvedKeys} chartColors={chartColors} fullscreen />
            </div>
          </div>,
          document.body
        )
...
export function parseChartBlocks(content: string): { cleanContent: string; charts: ChartConfig[] } {
...
export function parseImageBlocks(content: string): { cleanContent: string; imagePrompts: string[] } {
